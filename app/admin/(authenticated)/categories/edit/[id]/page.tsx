...
const EditCategory = ({ params }: { params: { id: number } }) => {
    const { data: category } = useGetCategory(params.id);
    ...
    useEffect(() => {
      form.setValue('id',category?.id ?? 0)
    ...
    const onSubmit = async (formData: z.infer<typeof UpdateCategoryFormSchema>) => {
        try {
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)}>
                        <div className="flex space-x-3">
                            ...
                            <div className=' flex-grow'>
                                ...
                                <Button type='submit' disabled={isUpdatingCategory} className='p-3 text-dark font-medium bg-yellow rounded text-center w-full'>
                                    {isUpdatingCategory ? 'Updating category...' : 'Update category'}
                                </Button>
                            </div>
                        </div>
                    </form>
